...
// See this include file for documentation.
...
// Returns a new allocated repolist*.
repolist* new_repolist() {
  repolist* rp;
  rp = (repolist*) malloc(sizeof(repolist));
  rp->repo_count = 0;

  rp->repos = (repo**) malloc(sizeof(repo*) * 20); // TODO: max len of 20 for now...

  return rp;
}

// TODO: free the repo list.
...
repolist* parse_repofile(const char* filepath) {
  ...
  print_debugf("Looking though %d packages...\n", count);
  ...
  for (int i = 1; i < count; i++) {
    ...
    rl->repos[rl->repo_count]->bin_files = strdup(
	    ini_property_value(ini, i, dindex)
    );
    ...
    rl->repos[rl->repo_count]->sha1 = strdup(
	    ini_property_value(ini, i, dindex)
    );

    dindex = ini_find_property(ini, i, "version", -1);
    rl->repos[rl->repo_count]->version = strdup(
	    ini_property_value(ini, i, dindex)
    );
...
// TODO: take a context logger
repolist* get_all_packages() {
  char* repo_file = download_repo_index(false);
  if (repo_file == NULL) {
    fprintf(stderr, "Failed to get package path\n");
    exit(1);
  }
...
// TODO: this will be slow for lots of packages. Should use key-value
// hash-map.
